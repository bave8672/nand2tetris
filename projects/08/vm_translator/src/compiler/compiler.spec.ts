...
describe(`vm compiler`, () => {
    ...
    describe(`external tests`, () => {
        ...
        it(`SimpleFunction`, () => {
            runExternalTest(`../FunctionCalls/SimpleFunction`);
        });
        ...
        function runExternalTest(path: string, ...flags: string[]) {
            const name = path.replace(/^[^[a-zA-Z0-9]?(.+\/)?/, "");
            execSync(
                `node ./built/cli/cli.js --in ${path}/**/*.vm --out ${path}/${name}.asm ${flags.join(
                    " "
                )}`
            );
            execSync(`bash  ../../../tools/CPUEmulator.sh ${path}/${name}.tst`);
        }
    });
});
